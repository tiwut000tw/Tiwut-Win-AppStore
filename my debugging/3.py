...
def parse_choco_list_output(output):
    ...
    for line in output.strip().split('\n'):
        parts = line.split()
        if len(parts) == 2:
            results.append({"name": parts[0].strip(), "id": parts[0].strip(), "version": parts[1].strip(), "update_available": False})
    return results
...
def parse_scoop_list_output(output):
    ...
    for line in lines[2:]:
        ...
        parts = line.split()
        if len(parts) >= 2:
            results.append({"name": parts[0], "id": parts[0], "version": parts[1], "update_available": False})
    return results
...
class AppStore(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Python App Store v5")
    ...
    def load_settings(self):
        try:
            ...
            return {"winget": {"search_command": 'powershell -Command "winget search --query \\"{query}\\" --accept-source-agreements"',"install_command": 'powershell -NoExit -Command "winget install --id \\"{package_id}\\" --accept-source-agreements"',"update_command": 'powershell -NoExit -Command "winget upgrade --id \\"{package_id}\\" --accept-source-agreements"',"list_command": 'powershell -Command "winget list"',"search_parser": "winget_search", "list_parser": "winget_list"}, "chocolatey": {"search_command": 'powershell -Command "choco search {query} --limit-output --exact"',"install_command": 'powershell -NoExit -Command "choco install {package_id} -y"',"update_command": 'powershell -NoExit -Command "choco upgrade {package_id} -y"',"list_command": 'powershell -Command "choco list --local-only"',"search_parser": "choco_search", "list_parser": "choco_list"},"scoop": {"search_command": 'powershell -Command "scoop search {query}"',"install_command": 'powershell -NoExit -Command "scoop install {package_id}"',"update_command": 'powershell -NoExit -Command "scoop update {package_id}"',"list_command": 'powershell -Command "scoop list"',"search_parser": "scoop_search", "list_parser": "scoop_list"}}
    ...
    def stop_task(self, calling_button=None):
        ...
        if calling_button: calling_button.configure(state="normal")
    ...
    def start_install_or_update_thread(self, app_data, action_type, button_widget):
        ...
        self.start_task(button_widget) # Disable the button and start progress bar
        # Run the actual command in a separate thread so it doesn't freeze the GUI
    ...
    def install_or_update_worker(self, app_data, action_type, button_widget):
        ...
        message = ""
        try:
            # Using .run() here waits for the process to finish
            process = subprocess.run(command, shell=True, capture_output=True, text=True)
            if process.returncode == 0:
                success = True
            else:
                # Capture error for status message
                message = process.stderr.strip() or f"Process exited with code {process.returncode}"
                print(f"Error during {action_type} of {name}: {message}")
    ...
    def on_action_complete(self, button_widget, app_name, action_type, success, error_message):
        """Called on the main thread after an install/update action is complete."""
        self.stop_task(button_widget) # Re-enable button and stop progress bar
        if success:
            self.update_status(f"Successfully completed {action_type} for {app_name}!", "green")
            # Automatically refresh the list of installed apps to show changes
            if self.tab_view.get() == "Installed Apps":
                self.populate_installed_apps_tab()
        else:
            self.update_status(f"Failed to {action_type} {app_name}. {error_message}", "red")
